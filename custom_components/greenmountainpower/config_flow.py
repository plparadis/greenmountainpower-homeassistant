    ...
    def _show_form(self, user_input):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Required(CONF_ACCOUNT_NUMBER, default=defaults.get(CONF_ACCOUNT_NUMBER)):
                        vol.Coerce(int),
                    vol.Required(CONF_USERNAME, default=defaults.get(CONF_USERNAME, "")): str,
                    vol.Required(CONF_PASSWORD): str,
                    vol.Optional(
                        CONF_PRICE_PER_KWH, default=defaults.get(CONF_PRICE_PER_KWH, DEFAULT_PRICE_PER_KWH)
                    ): vol.Coerce(float),
                    vol.Optional(
                        CONF_BACKFILL_DAYS,
    ...
    async def async_step_user(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Optional(
                        CONF_PRICE_PER_KWH, default=data.get(CONF_PRICE_PER_KWH, DEFAULT_PRICE_PER_KWH)
                    ): vol.Coerce(float),
                    vol.Optional(
                        CONF_BACKFILL_DAYS, default=data.get(CONF_BACKFILL_DAYS, DEFAULT_BACKFILL_DAYS)
                    ): vol.Coerce(int),
                }
            ),
        )
